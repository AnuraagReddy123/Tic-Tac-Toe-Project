...
# Initialize policy
policy = np.zeros(len(states)) # State index -> Action index
...
for state in states:
    ...
    if state_index not in absorbing_states:
        actions_int = utils.available_actions(state)
        actions_list = [action_to_index[tuple(action)] for action in actions_int]
        policy[state_index] = random.choice(actions_list)
...
for i in range(num_iterations):
    ...
    while True:
        ...
        for state in states:
            ...
            if state_index not in absorbing_states:
                ...
                action_index = int(policy[state_index])
                value_function[state_index] = 0
                action_int = index_to_action[action_index]
                intermediate_state = state.copy()
                intermediate_state[action_int[0]][action_int[1]] = 1
                if utils.check_if_wins(intermediate_state, 1):
                    value_function[state_index] = 1
                elif utils.check_draw(intermediate_state):
                    value_function[state_index] = 0
                else:
                    for action_opp in utils.available_actions(intermediate_state):
                        next_state = intermediate_state.copy()
                        next_state[action_opp[0]][action_opp[1]] = -1
                        next_state_index = state_to_index[tuple(next_state.flatten())]
                        value_function[state_index] += probability_transition_matrix[state_index][action_index][next_state_index] * (reward_matrix[state_index][action_index][next_state_index] + gamma * value_function[next_state_index])
    ...
    for state in states:
        ...
        if state_index not in absorbing_states:
            ...
            max_val = -np.inf
            for action_int in actions_int:
                action_index = action_to_index[tuple(action_int)]
                val = 0
                intermediate_state = state.copy()
                intermediate_state[action_int[0]][action_int[1]] = 1
                if utils.check_if_wins(intermediate_state, 1):
                    ...
                    for action_opp in utils.available_actions(intermediate_state):
                        ...
                        next_state[action_opp[0]][action_opp[1]] = -1
                        next_state_index = state_to_index[tuple(next_state.flatten())]
                        val += probability_transition_matrix[state_index][action_index][next_state_index] * (reward_matrix[state_index][action_index][next_state_index] + gamma * value_function[next_state_index])
                if val > max_val:
                    max_val = val
