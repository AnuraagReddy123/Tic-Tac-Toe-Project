...
x_policy = np.load(join('..', 'RL_X', 'Params', 'policy.npy'))
o_policy = np.load(join('..', 'RL_O', 'Params', 'policy.npy'))
...
def choose_action(markers:list, player):
    ...
    if player == -1:
        ...
        state_index = x_state_to_index[tuple(markers.flatten())]
        action = index_to_action[int(x_policy[state_index])]
        intermediate_state = markers.copy()
        intermediate_state[action[0]][action[1]] = 1
        return intermediate_state
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if game_over == False:
            ...
            if player == 1:
                markers = choose_action(markers, player=1)
                print('Action by X: ', transpose_list(markers))
                player *= -1
                check_game_over()
                ...
                markers = choose_action(markers, player=-1)
                print('Action by O: ', transpose_list(markers))
                player *= -1
                check_game_over()
                pygame.event.post(pygame.event.Event(pygame.MOUSEMOTION))
