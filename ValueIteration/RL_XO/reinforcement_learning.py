...
# Get O's policy
o_policy = np.load(join('..', 'RL_O', 'Params', 'policy.npy')) # Maps state to action (action is a tuple)
o_state_to_index = np.load(join('..', 'RL_O', 'Params', 'state_to_index.npy'), allow_pickle=True).item()
o_index_to_state = np.load(join('..', 'RL_O', 'Params', 'index_to_state.npy'), allow_pickle=True).item()
...
for state in states:
    # If # of Xs == # of Os + 1
    if np.count_nonzero(state.flatten() == 1) == np.count_nonzero(state.flatten() == -1) + 1:
    ...
    if state_index not in absorbing_states:
        ...
        for action_int in actions_int:
            ...
            if utils.check_if_wins(intermediate_state, 1):
                ...
                o_policy_action = o_policy[o_state_to_index[tuple(intermediate_state.flatten())]]
                o_policy_action = index_to_action[o_policy_action]
                intermediate_state[o_policy_action[0]][o_policy_action[1]] = -1
                probability_transition_matrix[state_index][action_to_index[tuple(action_int)]][state_index] = 1
                if utils.check_if_wins(intermediate_state, -1):                    
                    reward_matrix[state_index][action_to_index[tuple(action_int)]][state_index] = -1
                elif utils.check_draw(intermediate_state):
